...
// Find the prepack section and replace it
const prepackStart = content.indexOf('const prepackProducts = generateProducts("prepack", [')
const prepackEnd = content.indexOf('])', prepackStart) + 2

if (prepackStart === -1 || prepackEnd <= 1) {
  console.log('Could not find prepack section')
  process.exit(1)
}

// Also find the comment line before it
const commentStart = content.lastIndexOf('// ─── 프리팩', prepackStart)

const newPrepackSection = `// ─── 프리팩 (23) ── Actual BR prepack menu ────────────────
...
const startPos = commentStart !== -1 ? commentStart : prepackStart
content = content.substring(0, startPos) + newPrepackSection + content.substring(prepackEnd)

fs.writeFileSync(filePath, content, 'utf-8')
console.log('Successfully updated prepack products in mock-data.ts')
console.log('Replaced old 30 products with 23 real BR prepack items')
